...
class Animation():
    def __init__(self, source, first_driving, generator, kp_detector, relative = True, adapt_movement_scale = True):
        self.source = torch.tensor(
            source[np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
        first_driving = torch.tensor(np.array(first_driving)[
                                     np.newaxis].astype(np.float32)).permute(0, 3, 1, 2)
    ...
    def __call__(self, driving_frame):
        with torch.no_grad():
            driving_frame = torch.tensor(np.array(driving_frame)[np.newaxis].astype(
                np.float32)).permute(0, 3, 1, 2)  # b.c,h,w
...
def main_cv2():
    ...
    out = cv2.VideoWriter(
        'out.avi', cv2.VideoWriter_fourcc(*'FFV1'), fps, (256, 256))
    for _ in trange(frame_count):
        _, driving_frame = video.read()
        driving_frame = img_color_resize(driving_frame)
        if animation is None:
            animation = Animation(
                source_image, driving_frame, generator, kp_detector)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_random_face", default = True, action = "store_true",
                        help = 'use thispersondoesnotexist.com face')
    parser.add_argument(
